...
### 🟡 雷 10：數字輸入框不要在每次鍵入時強制回填最小值

**症狀**：使用者想把「12」改成別的數字，先刪掉前面的 `1` 或清空重打時，欄位立刻自己跳回 `1`，變得很難編輯。

**原因**：`onChange` 直接做 `Math.max(1, Number(value))`，空字串會立刻被轉成 `1`，把使用者的刪除動作吃掉。

**正確做法**：
- 編輯中的數字欄位先用字串 state 保留原輸入
- `onBlur` 或送出前再補最小值、做數字轉換
- 參考 `app/ledger/_components/TransactionForm.tsx` 的 `recurringEndCountInput`
